...
app.on('before-quit', (event) => {
    ...
    const payload = { log_file_path: logFilePath };
    const scriptArgs = [scriptPath, 'get_ai_log_summary', '', JSON.stringify(payload), ''];
    const result = spawnSync('python', scriptArgs, { encoding: 'utf8' });

    if (result.status !== 0) {
        log.error(`[Main Process] Health summary generation failed: ${result.stderr}`);
    } else {
        log.info('[Main Process] Health summary generated successfully.');
        // Logic to save the summary file can be added here if needed
    }
});
...
ipcMain.on('run-script', (event, { functionName, payload = {} }) => {
    ...
    // RENAMED: Now calls the robust script_router.py
    const scriptPath = path.join(__dirname, 'scripts', 'script_router.py');
